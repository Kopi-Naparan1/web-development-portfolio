...
function ServiceCard({ service, index, total, mobile = false }) {
  // Derive rounding from position, not data
  const mobileRadius =
    index === 0 ? "rounded-t-2xl" : index === total - 1 ? "rounded-b-2xl" : "";

  const borderRadius = mobile ? mobileRadius : "rounded-lg";
